        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
            ...
            <div className="flex gap-2">
              <Input type="number" placeholder="0.00" min="0" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)} className="flex-1" />
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-3">
              {members.map((m) => (
                <label key={m.id} className="flex items-center gap-2 cursor-pointer rounded-lg border border-border px-3 py-2 transition-colors hover:bg-muted has-[:checked]:ring-2 has-[:checked]:ring-primary/30 has-[:checked]:border-primary/50">
                  <Checkbox checked={splitAmong.includes(m.id)} onCheckedChange={() => toggleSplit(m.id)} />

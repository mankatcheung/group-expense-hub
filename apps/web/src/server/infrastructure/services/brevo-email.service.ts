import type { IEmailService, SendPasswordResetEmailInput, SendTripInvitationEmailInput, SendTripAddedNotificationInput } from '../../application/ports/services/email.service';
...
async function sendEmail(apiKey: string, sender: { email: string; name: string }, to: string, subject: string, html: string) {
  const response = await fetch('https://api.brevo.com/v3/smtp/email', {
    ...
    // Emails are sent inside the request; without a bound, a hung Brevo API
    // keeps the serverless function running until its maxDuration.

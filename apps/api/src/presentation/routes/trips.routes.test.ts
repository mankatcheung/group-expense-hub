...
import { createTripsPlugin } from './trips.routes.js';
...
describe('createTripsPlugin', () => {
  ...
  describe('GET /', () => {
    ...
    it('returns 401 when not authenticated', async () => {
      const failAuth = vi.fn(async (_req: FastifyRequest, reply: FastifyReply) => {
        reply.status(401).send({ error: 'Unauthorized' });
      });
      const localApp = await buildTestApp(createTripsPlugin(makeDefaultDeps({ requireAuth: failAuth })), '/api/trips');
    ...
    it('returns formatted trip summaries from the use case', async () => {
      ...
      const localApp = await buildTestApp(createTripsPlugin(deps as any), '/api/trips');
      const res = await localApp.inject({ method: 'GET', url: '/api/trips' });
  ...
  describe('POST /', () => {
    ...
    it('returns 400 for an invalid body', async () => {
      app = await buildTestApp(createTripsPlugin(makeDefaultDeps() as any), '/api/trips');
    ...
    it('returns 429 when rate limited', async () => {
      ...
      app = await buildTestApp(createTripsPlugin(deps as any), '/api/trips');
    ...
    it('creates the trip and returns it', async () => {
      ...
      app = await buildTestApp(createTripsPlugin(makeDefaultDeps({ createTrip: mockCreateTrip }) as any), '/api/trips');
      const res = await app.inject({ method: 'POST', url: '/api/trips', payload: validBody });
      expect(res.statusCode).toBe(200);
